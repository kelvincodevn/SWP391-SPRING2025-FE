...
const TestManagementSystem2 = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedTest) {
      setTests(prev =>
        prev.map(test =>
          test.id === selectedTest.id ? { ...formData, id: test.id } : test
        )
      );
      toast.success("Test updated successfully");
    } else {
      setTests(prev => [...prev, { ...formData, id: Date.now() }]);
  ...
  const confirmDelete = () => {
    setTests(prev => prev.filter(test => test.id !== selectedTest.id));
...
export default TestManagementSystem2;
